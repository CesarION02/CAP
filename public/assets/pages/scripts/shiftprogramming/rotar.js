function rotateShift() {
    var semana = document.getElementById("pastWeek").value;
            ...
            for (var i = 0; data[2].length > i; i++) {
                ...
                for (var j = 0; data[5].length > j; j++) {
                    if (data[2][i].idDepart == data[5][j].idDepartment) {
                        auxIdJob[contJob] = data[5][j].idJob;
                        auxNameJob[contJob] = data[5][j].nameJob;
                        contJob++;
                    }
                }
                contJob = 0;
                ...
                if (data[2][i].status == 2) {
                    ...
                    for (var j = 0; data[4].length > j; j++) {
                        if (data[2][i].group == data[4][j].idShift) {
                            ...
                            auxOrder[contWork] = data[4][j].rotate;
                            auxWork[contWork] = data[4][j].idWork;
                            if (data[4][j].rotate != 0) { contWorksin++; }
                            contWork++;
                        }
                    }
                    contWork = 0;
                    ...
                    for (var z = 0; auxIdJob.length > z; z++) {
                        ...
                        for (var x = 0; auxWork.length > x; x++) {
                            ...
                            for (var y = 0; data[1].length > y; y++) {
                                for (var r = 0; auxWork.length > r; r++) {
                                    if (data[1][y].id == auxWork[r]) {
                                        if (auxOrder[r] != 0) {
                                            if (auxOrder[r] + 1 > contWorksin) {
                                                auxSuma = 1;
                                            } else {
                                                auxSuma = auxOrder[r] + 1;
                                            }
                                        } else {
                                            auxSuma = 0;
                                        }
                                ...
                                for (var r = 0; auxOrder.length > r; r++) {
                                    if (auxSuma == auxOrder[r]) {
                                        nuevoWork = auxWork[r];
                                    }
                                }
                                if (data[1][y].idJob == auxIdJob[z] && nuevoWork == auxWork[x] && data[1][y].idD == data[2][i].idDepart) {
