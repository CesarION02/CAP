function rotateShift() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            //cambiar el turno que tiene cada empleado
            for (i = 0; data[1].length > i; i++) {
                switch (data[1][i].id) {
                    case 1:
                        data[1][i].id = 3;
                        break;
                        ...
                        for (var k = 0; data[1].length > k; k++) {
                            ...
                            for (var x = 0; auxWork.length > x; x++) {
                                ...
                                if (!turnHaveEmpl && sameDep && sameJob) {
                                    renglon[r] = renglon[r] + '<td style = "width: ' + 90 / con + '%;">' + crear_empleados(data[2][i].idDepart, auxWork[x], r + 1, auxIdJob[z], data[0], 0) + '</td>';
