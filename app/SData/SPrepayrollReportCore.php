...
class SPrepayrollReportCore {
    ...
    {
        $events = SPrepayrollReportCore::checkEvents();
        $comments = commentsControl::where('is_delete',0)->select('key_code','value')->get();

        $data53 = SPrepayrollReportCore::getSchedulesAndChecks($sStartDate, $sEndDate, $payWay, $lEmployees, $comments);

        $aEmployees = $lEmployees->pluck('id');
        $isCollection = true;
        $lWorkshifts = SDelayReportUtils::getWorkshifts($sStartDate, $sEndDate, $payWay, $aEmployees, $isCollection);
        $oAbsStartDate = Carbon::parse($sStartDate);
        $oAbsEndDate = Carbon::parse($sEndDate);
        $oAbsStartDate->subDay();
        $oAbsEndDate->addDay();
        $lAllAbsences = prePayrollController::getAllAbsences($oAbsStartDate->toDateString(), $oAbsEndDate->toDateString());
        $lAllHolidays = SPrepayrollReportCore::getAllHolidays($oAbsStartDate->toDateString(), $oAbsEndDate->toDateString());
        // Rutina para verificación de renglones completos
        // $lDataComplete = SPrepayrollReportCore::completeDays($sStartDate, $sEndDate, $data53, $aEmployees, $lWorkshifts);
        $lData53_2 = SPrepayrollReportCore::addEventsDaysOffAndHolidays($data53, clone $lWorkshifts, clone $lAllAbsences, clone $lAllHolidays, $comments);
        
        $aEmployeeBen = $lEmployees->pluck('ben_pol_id', 'id');
        $lDataWithAbs = SPrepayrollReportCore::addAbsences($lData53_2, $aEmployeeBen, $comments);

        // $aEmployeeOverTime = $lEmployees->pluck('is_overtime', 'id');
        $aEmployeeOverTime = $lEmployees->pluck('policy_extratime_id', 'id');
        $lData = SPrepayrollReportCore::addDelaysAndOverTime($lDataWithAbs, $aEmployeeOverTime, $sEndDate, $comments);

        $lDataWSun = SPrepayrollReportCore::addSundayPay($lData);
    ...
    public static function getSchedulesAndChecks($sStartDate, $sEndDate, $payWay, $lEmployees, $comments = null)
    {
        ...
        $isCollection = true;
        $lWorkshifts = SDelayReportUtils::getWorkshifts($sStartDate, $sEndDate, $payWay, $aEmployees, $isCollection);
        $lAllAssigns = SDelayReportUtils::getAllAssigns($sStartDate, $sEndDate, $isCollection);
        ...
        foreach ($lEmployees as $oEmployee) {
            ...
            foreach ($aDates as $sDate) {
                ...
                $registries = SPrepayrollReportCore::manageCheks($registries, $sDate, $lWorkshifts, $lAllAssigns);

                $bug = false;
                if (sizeof($registries) == 1) {
                    $res = SPrepayrollReportCore::manageOneCheck($sDate, $idEmployee, $registries, $lWorkshifts, $lAllAssigns);
                ...
                if (sizeof($registries) > 0) {
                    foreach ($registries as $registry) {
                        $theRow = SPrepayrollReportCore::manageRow($newRow, 
                                                                    $isNew, 
                                                                    $idEmployee, 
                                                                    $registry, 
                                                                    clone $lWorkshifts, 
                    ...
                    $result = SDelayReportUtils::getSchedule($sDate, $sDate, $idEmployee, $registry, clone $lWorkshifts, clone $lAllAssigns, \SCons::REP_HR_EX);
                    ...
                    $otherRow = SPrepayrollReportCore::setDates($result, $otherRow, $sDate, $comments);
            ...
            if (! $isNew) {
                if ($newRow->inDate == $sEndDate) {
                    ...
                    $theRow = SPrepayrollReportCore::manageRow($newRow, $isNew, $idEmployee, $registry, clone $lWorkshifts, clone $lAllAssigns, $sStartDate, $sEndDate, $comments);
                    $isNew = [0];
                    $newRow = $theRow[1];
                    $again = $theRow[2];
                    $fRegistry = $theRow[3];
                    ...
                    if ($again) {
                        if ($fRegistry != null) {
                            $theRow = SPrepayrollReportCore::manageRow($newRow, $isNew, $idEmployee, $fRegistry, clone $lWorkshifts, clone $lAllAssigns, $sStartDate, $sEndDate, $comments);
                        }
                        else {
                            $theRow = SPrepayrollReportCore::manageRow($newRow, $isNew, $idEmployee, $registry, clone $lWorkshifts, clone $lAllAssigns, $sStartDate, $sEndDate, $comments);
                        }
   ...
   /**
     ...
     */
    private static function manageRow($newRow, $isNew, $idEmployee, $registry, $qWorkshifts, $lAllAssigns, $sStartDate, $sEndDate, $comments = null)
    {
        if ($isNew) {
        ...
        if ($registry->type_id == \SCons::REG_OUT) {
            if (! $isNew) {
                $b24Hr = SPrepayrollReportCore::gapInAndOut24Hr($newRow->inDateTime, $registry->date.' '.$registry->time);
                if ($b24Hr) {
            ...
            $result = SDelayReportUtils::getSchedule($sStartDate, $sEndDate, $idEmployee, $registry, clone $qWorkshifts, clone $lAllAssigns, \SCons::REP_HR_EX);
            ...
            if ($result == null) {
                if ($isNew) {
                    ...
                    $otherResult = SDelayReportUtils::getNearSchedule($registry->date, $registry->time, $idEmployee, clone $qWorkshifts, clone $lAllAssigns);
                    $isNew = true;
                    ...
                    if ($otherResult != null) {
                        ...
                        $newRow = SPrepayrollReportCore::setDates($otherResult, $newRow, null, $comments);
                    }
                    else {
                ...
                if ($newRow->inDate != null) {
                    if ($newRow->outDate == null) {
                        $newRow = SPrepayrollReportCore::setDates($result, $newRow, null, $comments);
    ...
    private static function setDates($result, $oRow, $sDate = null, $comments = null)
    {
        if ($result == null) {
            ...
            $oRow->scheduleFrom = SPrepayrollReportCore::getOrigin($result);
            ...
            $oRow = SPrepayrollReportCore::checkTypeDay($result, $oRow);
        }
    ...
    public static function checkTypeDay($result, $oRow)
    {
        if ($result->auxWorkshift != null) {
            ...
            $oRow = SPrepayrollReportCore::setTypeDay($result->auxWorkshift->type_day_id, $oRow);
        }
     ...
     */
    public static function addEventsDaysOffAndHolidays($lData53, $qWorkshifts, $lAllAbsences, $lAllHolidays, $comments = null)
    {
        foreach ($lData53 as $oRow) {
            ...
            $lAbsences = prePayrollController::getAbsence($oRow->idEmployee, $sDt->toDateString(), clone $lAllAbsences);
            ...
            $holidays = SPrepayrollReportCore::getHolidaysOfDate($sOutDt->toDateString(), clone $lAllHolidays);
            ...
            foreach ($events as $event) {
                $oRow = SPrepayrollReportCore::setTypeDay($event->type_day_id, $oRow);
            }
        }
    ...
    public static function getAllHolidays($startDate, $endDate)
    {
        $lAllHolidays = \DB::table('holidays AS h')
                            ->select('h.id AS h_id', 'h.fecha', 'h.name')
                            ->whereBetween('h.fecha', [$startDate, $endDate])
                            ->where('h.is_delete', false)
                            ->distinct('h_id')
                            ->get();

        return collect($lAllHolidays);
    }

    public static function getHolidaysOfDate($sDt, $lAllHolidays)
    {
        $lHolidays = $lAllHolidays->where('fecha', $sDt);

        return $lHolidays->toArray();
    }
        ...
        foreach ($lData as $oRow) {
            ...
            if ($oRow->hasChecks) {
                ...
                if (! $oRow->isSpecialSchedule) {
                    if ($oRow->hasCheckIn) {
                        $mayBeOverTime = false;
                        $cIn = SPrepayrollReportCore::isCheckSchedule($oRow->inDateTime, $oRow->inDateTimeSch, $mayBeOverTime);
                    }
                    if ($oRow->hasCheckOut) {
                        //$mayBeOverTime = $aEmployeeOverTime[$oRow->idEmployee];
                        $mayBeOverTime = false;
                        $cOut = SPrepayrollReportCore::isCheckSchedule($oRow->outDateTime, $oRow->outDateTimeSch, $mayBeOverTime);
                    }
                    if (($cIn && $cOut) || ! $oRow->hasSchedule) {
                        $oRow = SPrepayrollReportCore::determineSchedule($oRow, $sEndDate, $workshifts);
                    }
                }

                // minutos de retardo
                $mins = SPrepayrollReportCore::getDelayMins($oRow->inDateTime, $oRow->inDateTimeSch);
                if ($mins > 0) {
                    $hasDelay = true;
                ...
                if ($oRow->hasCheckOut) {
                    $oRow->prematureOut = SPrepayrollReportCore::getPrematureTime($oRow->outDateTime, $oRow->outDateTimeSch);
                }
                else {
                ...
                $bWork8hr = true;
                $oRow->hasWorkedJourney8hr = SPrepayrollReportCore::journeyCompleted($oRow->inDateTime, $oRow->inDateTimeSch, $oRow->outDateTime, $oRow->outDateTimeSch, $bWork8hr, $oRow->workJourneyMins);
                $bWork8hr = false;
                $bJorneyCompleted = SPrepayrollReportCore::journeyCompleted($oRow->inDateTime, $oRow->inDateTimeSch, $oRow->outDateTime, $oRow->outDateTimeSch, $bWork8hr, $oRow->workJourneyMins);
                if ($bJorneyCompleted) {
                    ...
                    if ($aEmployeeOverTime[$oRow->idEmployee] == 2 || ($aEmployeeOverTime[$oRow->idEmployee] == 3 && $extendJourney->diffMinutes > 480)) {
                        ...
                        $oRow->overWorkedMins = SPrepayrollReportCore::getOverTime($oRow->inDateTime, $oRow->inDateTimeSch, $oRow->outDateTime, $oRow->outDateTimeSch);
    ...
    public static function getOverTime($inDateTime, $inDateTimeSch, $outDateTime, $outDateTimeSch)
    {
        ...
        $delay = SPrepayrollReportCore::getDelayMins($inDateTime, $inDateTimeSch);
     ...
     */
    public static function manageCheks($lCheks, $sDate, $lWorkshifts, $lAllAssigns)
    {
        if (sizeof($lCheks) == 0) {
            return $lCheks;
        }
        ...
        $result = SDelayReportUtils::getSchedule($sDate, $sDate, $auxCheck->employee_id, $registry, clone $lWorkshifts, clone $lAllAssigns, \SCons::REP_HR_EX);
        if ($result == null || ($result->auxScheduleDay != null && !$result->auxScheduleDay->is_active)) {
            return SPrepayrollReportCore::filterDoubleCheks($lCheks);
        }
        ...
        return SPrepayrollReportCore::filterDoubleCheks($lNewChecks);
    }

    private static function manageOneCheck($sDate, $idEmployee, $registries, $lWorkshifts, $lAssigns)
    {
        $oRegistry = $registries[0];
        $result = SDelayReportUtils::getSchedule($sDate, $sDate, $idEmployee, $oRegistry, clone $lWorkshifts, clone $lAssigns, \SCons::REP_HR_EX);
        ...
        foreach ($aEmployees as $idEmployee) {
            ...
            foreach ($aDates as $sDate) {
                ...
                if ($dayRows->count() == 0) {
                    ...
                    $oNewRow = SPrepayrollReportCore::setDates($result, $oNewRow, $sDate, $comments);
